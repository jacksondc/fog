...
function isEdited(filepath, content, callback) {
	if(filepath) {
		fs.readFile(filepath, function (err, data) {
			var savedContent = data.toString();
			if (err) {
				callback(true); //if there's no file, it must have been changed
			} else {
				callback(content !== savedContent);
			}
		});
	} else {
		throw new Error('isEdited requires a filepath');
	}
}
...
function closeHandler(e) {
	getFilepathAndContent(function(filepath, content) {
		if(filepath) {
			isEdited(filepath, content, function(edited) {
				resolveClose(edited, content);
			});
		} else {
